...
- Server : https://ochobotweb.ajajja.repl.co/
...
try {
query = args.join(' ')
buff = await getBuffer(`https://api.chipa.xyz/api/textpro/blackpink?text=${query}&apikey=${chipa}`)
dha.sendMessage(from, buff, image, { quoted: mek, thumbnail: Buffer.alloc(0)})
} catch (e) {
reply(`${e}`)
}
...
break
case 'suit':
if (isLimit(sender)) return reply(limitend(pushname)) //nyolong dari zenz
...
break
       case 'leaderboard':
if (isLimit(sender)) return reply(limitend(pushname)) //Cek Leaderboard
       case 'leaderboards':
if (isLimit(sender)) return reply(limitend(pushname))
              if (!isGroup) return reply(mess.only.group)
              if (!isLevelingOn) return await reply('Fitur leveling belum diaktifkan!') 
...
teks = `W h a t s a p p - B o t`
teks2 = `${a}Bot Rules${a} :\n${a}- Jangan spam bot${a}!\n${a}- Jangan telfon/videocall bot!${a}\n\n${a}Jika bot error chat owner bot!${a}\n\n${a}Jika anda menggunakan WhatsApp Mod / button tidak ada ketik #allmenu${a}\n\n${a}Thanks To \n- WandyGans\n- Rian${a}\n\n\n${a}Regards : WandyGans${a}`
but = [
